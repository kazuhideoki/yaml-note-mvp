...
1. **機能的条件**
   ...
   - [ ] Markdownのフロントマターから`schema_path`を抽出できる
   - [ ] 指定されたスキーマを自動的にロードできる
   - [ ] Markdownの見出し構造をYAML形式に変換できる
   - [ ] 変換されたYAMLをスキーマで検証できる
   - [ ] スキーマ違反がある場合、黄色のエラーバッジが表示される
   - [ ] 編集内容を変更すると、リアルタイム（100ms以内）で検証結果が更新される
   - [ ] `validated: false`の場合、スキーマ検証が無効化される

2. **テスト条件**

   - [ ] 単体テストが成功する（`pnpm test`）
   - [ ] スキーマ関連のユーティリティテストが成功
   - [ ] バリデーションフックのテストが成功

3. **コード品質条件**
   - [ ] `pnpm lint`で警告がないこと
   - [ ] `pnpm typecheck`で型エラーがないこと
   - [ ] JSDocドキュメントコメントが適切に記述されていること（日本語）
